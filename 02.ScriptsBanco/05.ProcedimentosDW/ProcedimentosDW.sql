...
BEGIN
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT ID FROM DIM_PRODUTO WHERE COD = @ID_PRODUTO)
		BEGIN
...
BEGIN
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT ID FROM DIM_ADICIONAL WHERE ID = @ID_ADICIONAL)
		BEGIN
...
BEGIN
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT S.ID FROM DIM_SABOR S WHERE S.ID = @ID_SABOR )
		BEGIN
...
BEGIN
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT L.ID FROM DIM_LOCAL L WHERE L.ID = @ID_LOCAL)
		BEGIN
...
BEGIN
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT E.COD FROM DIM_ESTABELECIMENTO E WHERE E.COD = @ID_ESTABELECIMENTO)
		BEGIN
...
EXEC ETL_TODOS_PROCEDURE '2020-01-01'
